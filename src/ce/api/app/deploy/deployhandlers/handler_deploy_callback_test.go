...
func (s *HandlerDeployCallbackSuite) Test_ExitCode_Success() {
	...
	s.Equal(uploadResult.Client.Location, d.StorageLocation.ValueOrZero())
	s.Equal(uploadResult.Client.FilesUploaded, d.S3NumberOfFiles.ValueOrZero())
	s.Equal(uploadResult.Client.BytesUploaded, d.S3TotalSizeInBytes.ValueOrZero())
	s.Empty(d.FunctionLocation.ValueOrZero())
	s.Empty(d.ServerPackageSize.ValueOrZero())
	s.Empty(d.APILocation.ValueOrZero())
	s.Empty(d.APIPackageSize.ValueOrZero())
	s.False(d.APIPackageSize.Valid)
...
func (s *HandlerDeployCallbackSuite) Test_ExitCode_EmptyManifest() {
	...
	s.Equal(uploadResult.Client.FilesUploaded, d.S3NumberOfFiles.ValueOrZero())
	s.Equal(uploadResult.Client.BytesUploaded, d.S3TotalSizeInBytes.ValueOrZero())
	s.Equal(uploadResult.Server.Location, d.FunctionLocation.ValueOrZero())
	s.Equal(uploadResult.Server.BytesUploaded, d.ServerPackageSize.ValueOrZero())
	s.Equal(uploadResult.API.Location, d.APILocation.ValueOrZero())
	s.Equal(uploadResult.API.BytesUploaded, d.APIPackageSize.ValueOrZero())
